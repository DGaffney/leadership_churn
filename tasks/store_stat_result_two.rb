class StoreStatResultTwo
  ...
  def perform(net)
    ...
    sr2 = StatResultTwo.first_or_create(hashtag: net["hasthag"], end_time: end_time, net_statistic: net["net_statistic"]||"indegree", strftime_template: net["strftime_template"])
    sr2.tau = net["tau"].nan? ? 0 : net["tau"]
    sr2.n = net["n"]
    sr2.index = net["index"]
    sr2.article_count = MediaCloudArticle.where(hashtag: net["hasthag"], :publish_date.lte => end_time).count
