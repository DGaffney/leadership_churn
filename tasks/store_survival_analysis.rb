class StoreSurvivalAnalysis
  ...
  def perform(record)
    sar = SurvivalAnalysisRecord.first_or_create(hashtag: record["hashtag"], timestamp: record["timestamp"], strftime: record["strftime_template"], metric_name: record["metric_name"])
